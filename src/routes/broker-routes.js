...
import { validateModifyPositionDTO } from '../models/dtos.js';
...
export default function brokerRoutes({
  ...
  router.post('/broker/positions/modify', requireBrokerWrite, async (req, res) => {
    ...
    try {
      const parsed = (() => {
        try {
          return validateModifyPositionDTO(req.body || {});
        } catch (error) {
          // Defer Zod error details to global handler if desired, but keep status 400 here.
          error.code = error.code || 'INVALID_PAYLOAD';
          throw error;
        }
      })();
      ...
      if (error?.name === 'ZodError' || error?.code === 'INVALID_PAYLOAD') {
        return badRequest(res, 'Invalid modify payload', { details: error?.issues || undefined });
      }
