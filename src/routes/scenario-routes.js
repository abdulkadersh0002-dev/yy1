...
import { buildLayeredAnalysis } from '../analyzers/layered-analysis.js';
import { computeIntermarketCorrelation } from '../services/analysis/intermarket-correlation.js';
...
const computeDecisionProbabilities = (signal, market) => {
  ...
  // Convert model outputs into a conservative probability distribution.
...
export default function scenarioRoutes({
  ...
  router.get('/scenario/analyze', requireSignalsGenerate, async (req, res) => {
    ...
    if (!parsed.success) {
      return badRequest(res, 'Invalid scenario request');
    }

    const { pair, symbol, broker, analysisMode, eaOnly } = parsed.data;
    ...
    try {
      ...
      const wantsEaOnly =
        brokerIsEa || eaOnly === true || String(analysisMode || '').toLowerCase() === 'ea';
      ...
      if (brokerIsEa) {
        const connected = eaBridgeService?.isBrokerConnected
          ? eaBridgeService.isBrokerConnected({ broker: brokerId, maxAgeMs: 2 * 60 * 1000 })
          : false;
        if (!connected) {
        ...
        if (eaBridgeService && typeof eaBridgeService.getQuotes === 'function') {
          ...
          const freshQuotes = eaBridgeService.getQuotes({ broker: brokerId, maxAgeMs });
          ...
          const resolvedSymbol =
            typeof eaBridgeService.bestSymbolMatch === 'function'
              ...
              : typeof eaBridgeService.resolveSymbolFromQuotes === 'function'
                ? eaBridgeService.resolveSymbolFromQuotes(brokerId, requestedPair, { maxAgeMs })
                : requestedPair;
          ...
          if (!hasQuoteForPair) {
            ...
            try {
              ...
              if (
                ...
                return res.status(409).json({
                  success: false,
                  error: `Waiting for live ${brokerId.toUpperCase()} price for ${requestedPair}`,
                  broker: brokerId,
                  pair: requestedPair,
                  resolvedSymbol
                });
          ...
          const refreshedQuotes = eaBridgeService.getQuotes({ broker: brokerId, maxAgeMs });
          ...
          return res.status(409).json({
            success: false,
            error: `EA quotes unavailable for broker ${brokerId.toUpperCase()}`,
            broker: brokerId
          });
        }
      }
      ...
      const options = wantsEaOnly
        ? { broker: brokerId || 'mt5', eaOnly: true, analysisMode: 'ea' }
        : {
            ...(broker ? { broker } : null),
