...
export default function eaBridgeRoutes({
  ...
  const buildServerPolicy = (broker) => {
    const requireRealtimeData = String(process.env.REQUIRE_REALTIME_DATA || '')
      .trim()
      .toLowerCase();
    const allowSyntheticData = String(process.env.ALLOW_SYNTHETIC_DATA || '')
      .trim()
      .toLowerCase();
    ...
    const envAllowWaitRaw = String(process.env.EA_SIGNAL_ALLOW_WAIT_MONITOR || '')
      .trim()
      .toLowerCase();
    const allowWaitMonitorExecution =
      envAllowWaitRaw === '1' || envAllowWaitRaw === 'true' || envAllowWaitRaw === 'yes';
    ...
    return {
      ...
      runtime: {
        requireRealtimeData:
          requireRealtimeData === '1' ||
          requireRealtimeData === 'true' ||
          requireRealtimeData === 'yes',
        allowSyntheticData:
          allowSyntheticData === '1' ||
          allowSyntheticData === 'true' ||
          allowSyntheticData === 'yes',
        allowAllSymbols:
          String(process.env.ALLOW_ALL_SYMBOLS || '')
            .trim()
            .toLowerCase() === 'true' || String(process.env.ALLOW_ALL_SYMBOLS || '').trim() === '1'
      }
    };
  };

  const isBackgroundSignalsEnabled = () => {
    const raw = String(process.env.EA_BACKGROUND_SIGNALS || '')
      .trim()
      .toLowerCase();
  ...
  const isScanAllowAllSymbolsEnabled = () => {
    const raw = String(process.env.EA_SCAN_ALLOW_ALL_SYMBOLS || '')
      .trim()
      .toLowerCase();
    ...
    const allowAll = String(process.env.ALLOW_ALL_SYMBOLS || '')
      .trim()
      .toLowerCase();
  ...
  router.post('/broker/bridge/:broker/market/bars', async (req, res) => {
    try {
      ...
      let payload;
      try {
        payload = validateMarketBarsIngestDTO(candidate);
      } catch (error) {
        return badRequest(res, { message: error?.message || 'Invalid bars payload' });
      }
