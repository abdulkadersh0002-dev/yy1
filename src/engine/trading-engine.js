...
class TradingEngine {
  ...
  validateSignal(signal) {
    ...
    // Market-data confidence floor becomes a soft penalty unless hard-blocked.
    const dataQualityPenalty = marketData?.confidenceFloorBreached ? 0.82 : 1.0;
    ...
    const confluence = (() => {
      ...
      const dqOk = dq?.confidenceFloorBreached ? false : true;
      addLayer(
        'data_quality',
        'Data quality confidence floor',
        0.6,
        dq == null ? 'SKIP' : dqOk ? 'PASS' : 'FAIL',
        dq == null ? 'No data-quality report' : dqOk ? null : 'Confidence floor breached',
        Number.isFinite(dqScore) ? { score: dqScore } : null
      );
