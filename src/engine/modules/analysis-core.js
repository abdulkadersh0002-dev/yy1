...
export const analysisCore = {
  ...
  combineAnalyses(pair, analyses, marketPrice, dataQualityReport = null) {
    ...
    const ensemble = this.adaptiveScorer
      ? this.adaptiveScorer.score(pair, {
          economic,
          news,
          technical
        })
    ...
    if (ensemble && ensemble.direction && ensemble.direction !== 'NEUTRAL') {
      if (directionPreQuality === 'NEUTRAL') {
        directionPreQuality = ensemble.direction;
      } else if (directionPreQuality !== ensemble.direction) {
        if (Math.abs(ensemble.finalScore) >= this.config.directionThreshold * 1.4) {
          directionPreQuality = ensemble.direction;
        }
      }
    }
    ...
    if (
      ...
      ensemble &&
      ensemble.direction &&
      ensemble.direction !== 'NEUTRAL'
    ) {
      if (direction === 'NEUTRAL') {
        direction = ensemble.direction;
      } else if (direction !== ensemble.direction) {
        if (Math.abs(ensemble.finalScore) >= this.config.directionThreshold * 1.4) {
          direction = ensemble.direction;
        }
      }
    ...
    let confidence = ensemble
      ? Math.min(100, baseConfidence * 0.55 + ensemble.confidence * 0.45)
      : baseConfidence;
    ...
    return {
      ...
      components: {
        ...
        adaptive: ensemble
          ? {
              probability: Number(ensemble.probability.toFixed(4)),
              ruleScore: Number(ensemble.ruleScore.toFixed(2)),
              ruleProbability: Number(ensemble.ruleProbability.toFixed(4)),
              modelProbability: Number(ensemble.modelProbability.toFixed(4)),
